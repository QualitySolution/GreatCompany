...
public class PlannedIncomeJournalViewModel : JournalViewModelBase<PlannedIncomeRow> {
	...
	public PlannedIncomeJournalViewModel(Repository repo, INavigationManager navigation, IInteractiveMessage interactive) : base(navigation, interactive) {
		this.repo = repo;
		Title = "План - приход";
	...
	protected override void Delete(PlannedIncomeRow row) { if(repo.Delete<PlannedIncome>(row.Id)) Reload(); else NotifyDeleteBlocked(); }
